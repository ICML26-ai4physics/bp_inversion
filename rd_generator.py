...
class RDGenerator(nn.Module):
    ...
    def _simulate_until_converged(
        ...
        p: GSParams,
        dt: float,
        tol: float,  # set tol to None if you want reaching max_steps counted as converged
        max_steps: int,
        ...
        for t in trange(max_steps,
            ...
            if tol is not None:
                ...
                u_next_a, v_next_a, _overflow = n_steps(u_a, v_a, p, dt, n=1)
                overflow = overflow or _overflow
                ...
                overflow = overflow or not torch.all(converged)

            else:
                u, v, overflow = n_steps(u, v, p, dt, n=1)
    ...
    def generate_gray_scott_target_batch(
        ...
        u = u.to(dev)
        v = v.to(dev)
        u, v, overflow, nstep = RDGenerator._simulate_until_converged(
    ...
    def simulate_to_steady_trunc_bptt(
        ...
        with torch.no_grad():
            u, v, overflow_early, steps_taken = self._simulate_until_converged(
                u=u, v=v, p=p, dt=dt, tol=tol, max_steps=max_steps
            )
        # TODO: Truncated BPTT can be temporarily disabled to use full BPTT.
        ...
        # TODO: Truncated BPTT can be temporarily disabled by commenting the below detach & extra steps code
        #  and decreasing tol passed into here.
        u = u.detach()
        v = v.detach()
        u, v, overflow_late = n_steps(u, v, p, dt, K, disable_progress_bar=False)
