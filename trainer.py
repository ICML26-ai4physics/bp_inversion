...
from power_spectrum_2d import ps_2d_loss, ps_2d_plus_ave_loss, VGGGramLoss
...
class Trainer:
    def __init__(self):
        ...
        self.gen = RDGenerator(GSParams(Du=0.16, Dv=0.08, F=0.050, k=0.060)).to(self.device)
        ...
        # self.vgg_loss = VGGGramLoss(self.device)
    ...
    def set_default_hyperparams(self):
        ...
        self.dt = 1.0
        self.unroll_K = 12000
        self.tol = None#5e-5# float('inf') #1e-3
        self.max_steps = 8000
    ...
    def train(self):
        ...
        def step_and_trial(idx, shrink=True):
            ...
            if ok:
                ...
                return False
        # End `def step_and_trial(idx, shrink=True)`
            ...
            try:
                ...
                if self.loss is not None:
                    ...
                    if debug_freq(it, print_more=1):
                        ...
                        '''
                        print(f"\n[it={it}] \033[36mGrads (last iteration's forward & backprop both done)\033[0m",
                ...
                # self.loss = self.vgg_loss(v_pred, v_t_target)
                ...
                with torch.no_grad():
                    p = self.gen.params_tensor()
                    print(
                        f"[it={it} (middle)] loss={self.loss.item():.6f} after forward, "
                ...
                if debug_freq(it, print_more=1):
                    ...
                    print(f"\033[32mPrediction v's std: min {pred_std.min().item():.04e}, median {pred_std.median().item():.04e}, max {pred_std.max().item():.04e}\033[0m")
                    print(f"\033[32mPower spectrum mean: {energy.item():.4e} (with log: {ps_mean:.4e})\033[0m\n")
